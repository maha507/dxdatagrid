	...
	onMount(async () => {
	  ...
	  const dataGrid = new DevExpress.ui.dxDataGrid(document.getElementById("dataGrid"), {
		...
		onRowInserting: async (e) => {
		  ...
		  try {
			const response = await fetch("https://api.recruitly.io/api/candidate?apiKey=TEST27306FA00E70A0F94569923CD689CA9BE6CA", {
			  method: "POST",
			  headers: {
		...
		onRowUpdating: async (e) => {
		  try {
			...
			const response = await fetch(`https://api.recruitly.io/api/candidate/${e.key.id}?apiKey=TEST9349C0221517DA4942E39B5DF18C68CDA154`, {
			  method: "POST",
			  headers: {
				"Content-Type": "application/json",
			  },
		...
		onRowRemoving: async (e) => {
		  console.log("Data being sent to API:", e.data);
		  try {
			const response = await fetch(`https://api.recruitly.io/api/candidate/${e.data.id}?apiKey=TEST27306FA00E70A0F94569923CD689CA9BE6CA`, {
			  method: "DELETE",
			});
			if (response.ok) {
